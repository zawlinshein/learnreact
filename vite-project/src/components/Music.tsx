import React, { FC, forwardRef, memo, useCallback, useEffect, useState } from "react";
...
const Music = memo(
    forwardRef<Props, { current: HTMLAudioElement }>(
        ({ src, dispatch, status, isPlaying, playNext }: Props, ref) => {
            ...
            const handleVolumeChange = useCallback(
                (event: React.ChangeEvent<HTMLInputElement>) => {
                    ...
                    console.log("volumne is recreating");
                    if (!isNaN(value) && ref.current) {
                        setVolume(value);
            ...
            function renderVolumeIcon(volume: number): React.JSX.Element {
                if (volume < 0.6) {
                    return <ImVolumeLow />;
                } else {
                    return <ImVolumeHigh />;
